...
if (settingsCount.count === 0) {
  ...
  insertSetting.run("installation_date", new Date().toISOString());
...
async function startServer() {
  ...
  app.post("/api/products", (req, res) => {
    const { name, category_id, purchase_price, sale_price, stock, min_stock, unit, brand, supplier_id, description, image } = req.body;
    ...
    // Find the highest number for this prefix to avoid duplicates
  ...
  app.post("/api/sales", (req, res) => {
    ...
    const transaction = db.transaction(() => {
      const saleInfo = db.prepare(`
        INSERT INTO sales (customer_id, total, subtotal, tax, payment_method)
        VALUES (?, ?, ?, ?, ?)
      `).run(customer_id, total, subtotal, tax, payment_method);
      ...
      for (const item of items) {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run(saleId, item.id, item.quantity, item.price, item.quantity * item.price);
